...
from tests.integration.data.jockey_match import (
    jockey_match,
    jockey_match_expected_data,
    jockey_unmatched_expected_data,
)
...
def test_jockey_match(load_test_data, fetch_test_data, assert_data_equal):
    ...
    unmatched_data = fetch_test_data("SELECT * FROM errors.staging_entity_unmatched")
    assert_data_equal(jockey_unmatched_expected_data, unmatched_data)
