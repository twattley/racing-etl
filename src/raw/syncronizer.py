...
class Syncronizer:
    ...
    def set_database(self):
        ...
        if self.job_name.startswith('tf'):
            self.log_file = os.path.join(os.getcwd(), 'src/raw/timeform', f'{self.job_name}.csv')
        else:
            self.log_file = os.path.join(os.getcwd(), 'src/raw/racing-post', f'{self.job_name}.csv')


    def pull_data(self):
        self.data = fetch_data(f'SELECT * FROM {self.database_schema}.{self.database_table}')
