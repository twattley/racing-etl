...
def remove_brackets_from_draw(data: pd.DataFrame) -> pd.DataFrame:
    return data.assign(
        draw=lambda x: x["draw"].str.extract(r"(\d+)")
    )
...
def get_tf_rating_values(data: pd.DataFrame) -> pd.DataFrame:
    ...
    data = data.assign(
        ...
        tf_rating_view=lambda x: x["tf_rating"].str.extract(r"(\D+)").map(view_map).fillna("neutral")
    )
    data = data.assign(
        tf_rating=pd.to_numeric(data["tf_rating"], errors="coerce").fillna(0).astype(int)
    )
