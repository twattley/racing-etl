...
def process_todays_betfair_entity_matching():
    ...
    for row in bf_data.itertuples():
        ...
        if sub_rp_data.empty:
            ...
            continue
        I(f"Found {sub_rp_data.shape[0]} RP data for {row.race_time} {row.course_id}")
    ...
    pt(
        lambda: db.store_data(matched_data, "entity_matches", "bf_raw", truncate=True),
